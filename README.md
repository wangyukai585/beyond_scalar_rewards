...
用 Gemini 作为外部 Oracle 替代传统 reward model，在 GPT-2 上实现无 critic 的轻量级 RL 训练。
...
```bash
cp .env.example .env
# 编辑 .env，填入真实 key：GEMINI_API_KEY=your_key
```
...
| `grpo_metrics.json` | GRPO 训练曲线（step, loss, kl, entropy...） |
